...
def send_telegram_notification(message, chat_id, bot_token):
    ...
    payload = {
        "chat_id": chat_id,
        "text": message,
        "parse_mode": "Markdown"  # Supports bold, italic, links
    }
...
# Function to scrape job postings from a URL
...
# Load seen REQ IDs from file
def load_seen_req_ids(file_name):
    try:
        with open(file_name, 'r') as f:
            data = json.load(f)
            return set(data.get('seen_req_ids', []))
    except (FileNotFoundError, json.JSONDecodeError):
        return set()

# Save updated REQ IDs to file
def save_seen_req_ids(req_ids, file_name):
    with open(file_name, 'w') as f:
        json.dump({'seen_req_ids': list(req_ids)}, f)

# Job notifier configuration: separate URL, seen file, and chat list
job_configs = [
    {
        "url": "https://careers.woolworthsgroup.com.au/en_GB/apply/search-jobs/?13316=6987&13316_format=18944&13321_location_place=Upper%20Mount%20Gravatt,%20Brisbane%20City,%20Queensland,%20Australia&13321_location_radius=20&13321_location_coordinates=[-27.56,153.08]&listFilterMode=1&jobSort=schemaField_3_78_3&jobSortDirection=DESC&jobRecordsPerPage=6&",  # Link 1
        "seen_file": "seen_req_ids_user1.json",
        "chat_ids": ["649377103", "7021757983", "6441296270"]  # User 1
    },
    {
        "url": "https://careers.woolworthsgroup.com.au/en_GB/apply/search-jobs/?13321_location_place=Melbourne,%20City%20of%20Melbourne,%20Victoria,%20Australia&13321_location_radius=20&13321_location_coordinates=[-37.81,144.97]&listFilterMode=1&jobSort=schemaField_3_78_3&jobSortDirection=DESC&jobRecordsPerPage=6&",  # Link 2
        "seen_file": "seen_req_ids_user2.json",
        "chat_ids": ["7021757983","7890180275","789821759"]  # User 2 and 3
    }
]
...
def check_new_job_postings_periodically():
    ...
    while True:
        ...
        for config in job_configs:
            ...
            seen_req_ids = load_seen_req_ids(config["seen_file"])
            ...
            if new_jobs:
                ...
                save_seen_req_ids(seen_req_ids, config["seen_file"])
            else:
                print(f"[{datetime.now()}] No new jobs found for {config['seen_file']}", flush=True)
